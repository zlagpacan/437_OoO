...
- target daxpy/vector add behavior with memory latency overlapping and large instruction windows
...
- palgorithm.asm is a producer-consumer parallel program with thread synchronization (so instruction count cannot be extracted from the program itself, varying by implementation and memory latency, and can be misleading). This file could not be included in the repo as it is an algorithm students develop as part of Purdue ECE 437. 
